try {
	! function() {
		...
		i.setPath("https://dup.baidustatic.com/"), i.define({
			...
			$config: function() {
				this.HTTP_PROTOCOL = "https:" === document.location.protocol ? "https:" : "http:", 0 !== location.protocol.indexOf("http") && (this.HTTP_PROTOCOL = "https:")
			}
		}), i.define({
			name: "lang",
			...
			expLog: function(t) {
				var e, i = t.id ? t.id : "",
					...
					d = t.exps ? t.exps : "";
				e = this.config.LOG_URL + ["?type=" + o, "id=" + i, "pos=" + r, "status=" + s, "async=" + n, "mes=" + a, "exps=" + d, "from=ssp", "tm=" + +new Date].join("&"), this.log.sendLogRequest(e)
			},
			sendLog: function(t) {
				if (t.response) {
			...
			getParamObj: function(t) {
				...
				if (t.width > 0 && t.height > 0 && (i.sz = t.width + "x" + t.height), this.browser.baiduboxapp) {
					...
					s.articleTitle && (i.ti = s.articleTitle), s.MP && s.MP.globalConf && (i.utdi = encodeURIComponent(window.MP.globalConf.cuid) || "", i.atdi = encodeURIComponent(window.MP.globalConf.nid) || "")
				}
			...
			getPmpRequestUrl: function(t) {
				...
				if (c) {
					...
					var m = c.pdb_deliv;
					m.deliv_id && "0" !== m.deliv_id && n.push("pdbid=" + m.deliv_id), c.media_protect && "0" !== c.media_protect && n.push("mpdi=" + c.media_protect);
					var f = c.order_deliv;
					f.deliv_id && "0" !== f.deliv_id && n.push("orderid=" + f.deliv_id), f.demand_id && "0" !== f.demand_id && n.push("odid=" + f.demand_id);
					var g = c.rtb_deliv;
					g.deliv_id && "0" !== g.deliv_id && n.push("rtbid=" + g.deliv_id), g.demand_id && "0" !== g.demand_id && n.push("rdid=" + g.demand_id);
			...
			processSlotInfo: function(t) {
				...
				if (1 === r) e.width = s, e.height = a;
				else if (2 === r || 5 === r) {
					t.pcwd || t.ftpc ? (e.wScale = t.pcwd || "-1", t.styleOpenApi.cpro_ftpc = "true") : e.wScale = s, t.pchd ? e.hScale = t.pchd : e.hScale = a;
					var c = {},
						h = t.styleOpenApi.cpro_ftpc || "true" === t.styleOpenApi.cpro_ftpc || "-1" === e.wScale;
					h = 5 === r ? !0 : h;
					var p = this.dom.g(t.containerId);
					h && p && p.parentElement.clientWidth && (c.width = p.parentElement.clientWidth || 0), "-1" === e.hScale && p && p.parentElement && (c.height = p.parentElement.clientHeight || 0);
					var u = this.param.translateScaleToPixelSize(e.wScale, e.hScale, c);
					o.width = u.width, o.height = u.height, t.width = u.width, t.height = u.height, e.width = u.width, e.height = u.height
				} else if (3 === r) {
				...
				var f = t.styleOpenApi,
					g = f.cpro_w || f.rsi0 || 0,
					y = f.cpro_h || f.rsi1 || 0;
				return (g || y) && (e.width = g || this.getCurViewportWidth(s), e.height = y || a, t.width = e.width, t.height = e.height), e
			}
		}), i.define({
			name: "detect",
			namespace: "dup.business",
