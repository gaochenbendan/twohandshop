...
public class UploadController {
    ...
    private String upload_path_icon;
...
//        爆存文件
        try {
            photo.transferTo(new File(upload_path+"/"+fileName));
        }catch (Exception e)
        {
    ...
    @RequestMapping(value="/upload_photo_icon",method= RequestMethod.POST)
    @ResponseBody
    public Result<String> uploadPhotoIcon(
            @RequestParam(name = "photo",required = true) MultipartFile photo
    )
    {
//        获取文件原始名
...
//        保存文件
        File filepath = new File(upload_path_icon);
//        判断文件是否存在
        if(!filepath.exists())
        {
            filepath.mkdir();
...
////        新建文件夹
//        filepath = new File(upload_path + "/" + StringUtil.getFormatterDate(new Date(), "yyyyMMdd"));
//        //        判断文件是否存在
//        if(!filepath.exists())
//        {
//            filepath.mkdir();
...
//        String fileName = StringUtil.getFormatterDate(new Date(), "yyyyMMdd") + "\\" + System.currentTimeMillis() + substring;
        String fileName =System.currentTimeMillis() + substring;
//        爆存文件
        try {
            photo.transferTo(new File(upload_path_icon+"/"+fileName));
        }catch (Exception e)
        {
            e.printStackTrace();
        }
